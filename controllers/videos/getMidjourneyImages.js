...
const getMidjourneyImages = async (req, res) => {
  try {
    const result = await VideoService.getMidjourneyImages(req.body.prompt, "")
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('getMidjourneyImages Error', err)
    return res.sendJson({
...
router.post('/getMidjourneyImages', validation, getMidjourneyImages)
