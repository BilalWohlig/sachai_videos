...
const finalImages = async (req, res) => {
  try {
    const data = req.body
    if(data.payload.status == 'in-progress') {
        console.log("Image Generation In Progress")

    }
    if(data.payload.status == 'completed') {
        // console.log("Received Final Image", data)
        await VideoService.createObjInDB(data)
    }
  } catch (err) {
    console.log('finalImages Error', err)
    return res.sendJson({
...
router.post('/finalImages', validation, finalImages)
