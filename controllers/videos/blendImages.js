...
const blendImages = async (req, res) => {
  try {
    const result = await VideoService.blendImages(req.body.images)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('blendImages Error', err)
    return res.sendJson({
...
router.post('/blendImages', validation, blendImages)
