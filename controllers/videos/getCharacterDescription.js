...
const getCharacterDescription = async (req, res) => {
  try {
    const result = await VideoService.getCharacterDescription(req.body.script)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: JSON.parse(result.choices[0].message.content) })
  } catch (err) {
    console.log('getCharacterDescription Error', err)
    return res.sendJson({
...
router.post('/getCharacterDescription', validation, getCharacterDescription)
