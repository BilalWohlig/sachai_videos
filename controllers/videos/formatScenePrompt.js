...
const formatScenePrompt = async (req, res) => {
  try {
    const result = await VideoService.formatScenePrompt(req.body.prompt)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('formatScenePrompt Error', err)
    return res.sendJson({
...
router.post('/formatScenePrompt', validation, formatScenePrompt)
